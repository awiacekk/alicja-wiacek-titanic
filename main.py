...
def main():
    st.set_page_config(page_title="Sprawdź czy przeżyłbyś katastrofę")
    overview = st.container()
    left, right = st.columns(2)
    prediction = st.container()

    st.image("https://static.prsa.pl/images/e5f114bd-c5fd-4b7c-8115-b660612500e5.jpg?format=500")

    with overview:
        st.title("Sprawdź czy przeżyłbyś katastrofę")

    with left:
        pclass_radio = st.radio("Klasa", list(sex_d.keys()), format_func=lambda x: sex_d[x])
        sex_radio = st.radio("Płeć", list(pclass_d.keys()), format_func=lambda x: pclass_d[x])
